...
export class AdoptionService extends BaseService {
  ...
  userAdoptDog(dog: Dog): Observable<DogAdopt> {
    ...
    // Send to Server
    return this.http
      .post<Response>(this.MainUrl + 'dog-adopt/user-dog-adapt', request)
      .pipe(
        map((response: Response) => {
          if (response.status == 'success') return response.result as DogAdopt;
  ...
  viewUserAdoptionHistory(): Observable<Array<DogAdopt>> {
    ...
    return this.http
      .post<Response>(
        this.MainUrl + 'dog-adopt/user-view-all-dog-adopt',
        request
      )
      .pipe(
        map((response: Response) => {
  ...
  adminConfirmAdoptDog(dog_adopt: DogAdopt): Observable<boolean> {
    ...
    return this.http
      .post<Response>(
        this.MainUrl + 'dog-adopt/admin-confirm-dog-adopt',
        request
      )
      .pipe(
        map((response: Response) => {
