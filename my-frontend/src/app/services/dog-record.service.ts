...
export class DogRecordService extends BaseService {
	...
	updateDogRecord(dog_id: number, updated_dog_info: Dog): Observable<Dog> {
		...
		for (const key in jsonData) {
			...
			if (key == "photoBytes") {
				formData.append(key, updated_dog_info.photoBytes, updated_dog_info.photoBytes.name);
				continue;
			}
		...
		return this.http.put<Dog>(this.MainUrl + 'dog/update-dog', formData, { headers: headers });
	}
